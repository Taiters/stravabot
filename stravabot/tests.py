...
class SlackStoreTests(TestCase):
    ...
    def test_consume_expired(self):
        for time_since_created, should_pass in [
            (120, False),
            (121, False),
            (119, True),
            (1, True),
            ...
            with self.subTest([time_since_created, should_pass]):
                ...
                with patch('stravabot.oauth.store.timezone.now') as now_mock:
                    ...
                    store = DjangoOAuthStateStore(120)
                    state = store.issue()
                self.assertEqual(store.consume(state), should_pass)
    
    def test_consume_modified(self):
        store = DjangoOAuthStateStore(120)
        state = store.issue() + "modified"
        self.assertFalse(store.consume(state))
    
    def test_store_team_installation(self):
...
class SlackOAuthTests(TestCase):
    def test_thing(self):
        result = self.client.get(reverse('slack_authorize') + urlencode({
            'code': 'abcd',
            'state':'defg',
        }))
